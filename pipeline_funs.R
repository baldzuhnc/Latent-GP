...
get_latest <- function(country) {
  ...
  files <- list.files(
    paste0("data/", country),
    pattern = paste0("^W\\d+\\_", country, ".rds$"),
    full.names = TRUE
  )
  if (length(files) == 0) {
...
check_vars <- function(data, vars) {
  ...
  max_values <- data |>
    summarise(across(all_of(vars), ~ max(.x, na.rm = TRUE))) |>
    as_vector()
...
subset_data <- function(data, cutoff_ids = NULL) {
  ...
  cat("\n====================================\n")
  cat(
    "Excluding observations with more than",
    missings_allowed,
    "missings in a row."
  )
  cat("\n====================================\n")
...
subset_to_long_df <- function(sdata) {
  ...
  ldata <- sdata %>%
    ...
    mutate(
      item = as.integer(factor(item, levels = colnames(sdata)[3:ncol(sdata)]))
    ) %>%
...
extract_posterior_summary <- function(model_fit, pid_map, wave_map) {
  ...
  cat("\n====================================\n")
  cat(
    "Peak RAM during posterior processing:",
    mem_usage2$Peak_RAM_Used_MiB[1],
    "MB\n"
  )
  cat("====================================\n")
...
extract_posterior_theta <- function(model_fit, n_draws = 200) {
  ...
  theta4_estim_raw <- model_fit %>%
    ...
    mutate(i = as.factor(i), t = as.factor(t))
...
save_gp_summary <- function(post_summary_df, title) {
  ...
  cat(paste0(
    "Saving GP posterior summary dataframe to ",
    gp_summary_path,
    title,
    ".rds",
    "\n"
...
plot_data <- function(ldata, dim, n_pids = NULL) {
  ...
  ggplot(
    data = data %>% group_by(.data[[dim]], Y) %>% summarize(Y_count = n())
  ) +
