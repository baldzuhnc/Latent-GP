...
plot_theta_trajectory <- function(post_df, y_lim = NA, plotly = F, title = "") {
  ...
  pocket_p <- ggplot() +
    geom_line(
      data = pocket_df,
      aes(x = wave, y = mean, group = pid, text = paste("pid:", pid)),
      alpha = 0.1
    ) +
  ...
  if (!any(is.na(y_lim))) {
    pocket_p <- pocket_p + ylim(y_lim)
  }
  if (plotly) {
    pocket_p <- plotly::ggplotly(pocket_p, tooltip = "text")
  }
...
plot_ml_vs_theta <- function(ldata, post_df, n_pids = 20) {
  ...
  p <- ggplot() +
    ...
    geom_line(
      data = post_sub %>% filter(parameter == "theta"),
...
remove_outliers <- function(post_df, theta_lower = -10, theta_upper = 10) {
  ...
  valid_pids <- post_df %>%
    ...
    group_by(pid) %>%
    summarise(
      valid = max(mean) < theta_upper & min(mean) > theta_lower,
      .groups = "drop"
    ) %>%
    filter(valid) %>%
    pull(pid)
  ...
  if (removed > 0) {
    cat(sprintf(
      "Note: Removed %d trajectories with extreme values. %d remain.\n",
      removed,
      length(valid_pids)
    ))
  }
